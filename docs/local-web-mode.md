...
连接成功后，网页的“Demo 资料库”支持直接选择一个本地文件夹。文件夹授权通过 Windows 原生窗口完成；后端递归扫描 `.dem`，计算 SHA-256 指纹并读取地图、补丁版本、玩家与 SteamID。网页可以筛选并选择一场比赛，不需要把每个大文件逐个上传。

- 原始 Demo 始终保留在原文件夹；
- 网页只收到相对路径和随机会话编号，不收到磁盘绝对路径；
- 文件夹授权保存在本机内存，默认一小时后自动失效；
- 重复或元数据读取失败的文件不能进入完整解析；
- 选择玩家后才执行逐回合分析，因此资料库扫描不会一次解析整个目录。
